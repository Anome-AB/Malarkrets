...
export function AdminClient({ users, tags }: AdminClientProps) {
  ...
  function handleAnonymize(userId: string, email: string) {
    if (!confirm(`Vill du anonymisera användaren ${email}? Detta kan inte ångras.`)) {
      return;
    }
  ...
  function handleDeleteTag(tagId: number, tagName: string) {
    if (!confirm(`Vill du ta bort taggen "${tagName}"? Den tas bort från alla användare och aktiviteter.`)) {
      return;
    }
